    ...
    if(url.includes("app.requestly.io")&&!url.includes("de_ad_before=daylight")&&toggle){
      ...
      if('initiator' in details){
        ...
        try{
          ...
          // dataA.push(features["url_77_max"]);
          // dataA.push(features['keyword_char_present']);
          // dataA.push(features["ng_2_2_15_2"]);
          // dataA.push(features["url_50_std"]);
          // dataA.push(features['num_get_cookie']);
          // dataA.push(features['num_requests_sent']);
          // dataA.push(features["ng_19_2_15_2"]);
          // dataA.push(features["url_144_std"]);
...
function featureExtract(url, requestHeader){
    ...
    return new Promise((resolve, reject) => {
      (async()=>{
          ...
          //   const squaredDiffSum = url2list.reduce(async (sum, char) => {
          //     const vec = fqdnVector[char];
          //     if (vec) {
          //       return (await sum).map((val, i) => val + Math.pow(vec[i] - returnFeatures['fqdnEmbedding'][i], 2));
          //     }
          //     return await sum;
          //   }, Promise.resolve(Array(vec_length).fill(0)));

          //   const maxVec = url2list.reduce(async (currentMax, char) => {
          //     const vec = fqdnVector[char];
          //     if (vec) {
          //       return (await currentMax).map((val, i) => Math.max(val, vec[i]));
          //     }
          //     return await currentMax;
          //   }, Promise.resolve(Array(vec_length).fill(-Infinity)));


          //   const avg = (await embedding).map((val) => val / url2list.length);
          //   const variance = (await squaredDiffSum).map((val) => val / url2list.length);
          //   const stdDev = variance.map((val) => Math.sqrt(val));
          //   const max = await maxVec;
          ...
          returnFeatures["url_77_max"]=url_maxVec[77];
          returnFeatures["url_41_mean"]=embedding2[41];
          returnFeatures["url_144_std"]=url_stdDev[144];
          returnFeatures["url_3_mean"]=embedding2[3];
          returnFeatures["url_50_std"]=url_stdDev[50];
          ...
          try{
            if (string_content_type == 'script') {
              ...
              if(js!=""){
                ...
                //     let ngram={};
                //     let ngramsum=0;
                //     if(gramsource.length>3){
                //       ngramsum=gramsource.length-3;
                //       for(let i=3;i<gramsource.length;i++){
                //         let pattern=String(ngramdict[gramsource[i-3]])+'_'+String(ngramdict[gramsource[i-2]])+'_'+String(ngramdict[gramsource[i-1]]+'_'+String(ngramdict[gramsource[i]]))
                //         if(pattern in capturingngrams){
                ...
                //     if("2_2_15_2" in ngram){
                //       returnFeatures["ng_2_2_15_2"]=ngram["2_2_15_2"]
                //     }
                //     if("19_2_15_2" in ngram){
                //       returnFeatures["ng_19_2_15_2"]=ngram["19_2_15_2"]
                //     }
              ...
              if(xhr!=""){
                ...
                if(script_blocks){
                  for(let script_block of script_blocks){
                    ...
                    //         if(pattern in capturingngrams){
                    //         // if(pattern=='0_0_2'||pattern=='0_15_15'||pattern=='2_13_2'||pattern=='15_0_3'||pattern=='15_0_15'||pattern=='15_15_15'){
                    //           ngramsum=ngramsum+gramsource.length-3;
                    //           if(pattern in ngram){
                    //             ngram[pattern]=ngram[pattern]+1;
                    //           }
                    //           else{
